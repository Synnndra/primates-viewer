...
async function fetchNFTs() {
    ...
    try {
        // Primates collection address
        const PRIMATES_COLLECTION = 'AAG6dgqaSdSToaCTyH8W6GBk6sZdu9jC3TUDfy7eLEmz';
        ...
        // Filter to only Primates collection
        const nfts = allItems.filter(item => {
            ...
            return collections.includes(PRIMATES_COLLECTION);
        });

        console.log(`✅ Primates: ${nfts.length}`);
        ...
        if (nfts.length === 0) {
            nftContainer.innerHTML = '<div class="empty-state">No Primates NFTs found in these wallets</div>';
            return;
        }
...
function groupByCollection(nfts) {
    ...
    nfts.forEach(nft => {
        ...
        // Use "Primates" as the display name
        const collectionName = 'Primates';
...
async function downloadCollage() {
    ...
    try {
        ...
        canvas.toBlob((blob) => {
            ...
            a.href = url;
            a.download = `primates-collage-${Date.now()}.png`;
...
function applyFilters() {
    ...
    console.log(`Showing ${visibleCount} of ${nftCards.length} NFTs`);
...
function clearAllFilters() {
    ...
    console.log(`Showing all ${nftCards.length} NFTs`);
